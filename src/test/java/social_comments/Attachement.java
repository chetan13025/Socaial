...
public class Attachement {
    ...
    private static String Attachment_endpoint = ConfigReader.getProperty("attachment_endpoint");
    ...
    private void captureDbSnapshot(int AttachmentId, String action) {
		ExtentTest current = ExtentCucumberListener.getCurrentScenario();
		if (current == null)
			return;
		...
		try (Connection conn = DriverManager.getConnection("jdbc:mysql://10.10.2.45:3306/social_comments_model_dhin",
				...
				ResultSet rs = stmt
						.executeQuery("SELECT * FROM `social_comments_model_dhin`.`Attachment` LIMIT 1000")) {
	...
	public void i_have_a_attachment() {
		requestBody = DataCreate.generateAttachmentJson(MessageId);
//		System.out.println("Resource JSON:\n" + requestBody);
	}
    @When("Create a Attachment using existing MessageId")
    public void create_a_attachment_using_existing_MessageId() {
        ...
        response = given().header("Content-Type", "application/json")
                ...
                .body(requestBody)
                .when()
                .post(baseUrl + Attachment_endpoint)
                .then()
                .extract()
                .response();

        // log response to console
        System.out.println("POST Attachment Status: " + response.getStatusCode());
        System.out.println("POST Attachment Body: " + response.getBody().asString());
        AttachmentId = response.jsonPath().getInt("id");
		System.out.println("Stored AttachmentID: " + AttachmentId);
        // log to extent
        logToExtent("Create a Attachment", requestBody, response);
        captureDbSnapshot(AttachmentId, "Create a Attachment");
    }
    @When("Fetch All Attachment Details")
  	public void Fetch_All_attachment_Details() {
  		response = given().header("Content-Type", "application/json").header("X-TENANT-ID", tenantId)
  				.header("Author", author).when().get(baseUrl + Attachment_endpoint).then().extract().response();

  		System.out.println("Response Body:\n" + response.getBody().asString());
  		System.out.println("Status Code: " + response.getStatusCode());

  		logToExtent("Fetch All Attachment Response", requestBody, response);
//          AllureLogger.logResponse("Fetch All Resources Response", response);
  	}
    @When("Fetch Attachment Details with ID")
  	public void Fetch_attchment_Details_with_ID() {
  		response = given().header("Content-Type", "application/json").header("X-TENANT-ID", tenantId)
  				.header("Author", author).pathParam("id", AttachmentId).when()
  				.get(baseUrl + Attachment_endpoint + "/{id}").then().extract().response();

  		System.out.println("Response Body:\n" + response.getBody().asString());
  		System.out.println("Status Code: " + response.getStatusCode());
//          AllureLogger.logResponse("Create Resource Response", response);
  		logToExtent("Fetch Attachment by ID", null, response);
  	}
...
//			throw new IllegalStateException("No ResourceID available. Run POST first.");
		...
		String newJson = DataCreate.generateAttachmentJson(MessageId);

		response = given().header("Content-Type", "application/json").header("X-TENANT-ID", tenantId)
				.header("Author", author).pathParam("id", AttachmentId).body(newJson).when()
				.put(baseUrl + Attachment_endpoint + "/{id}").then().extract().response();

		System.out.println("PUT Response Body:\n" + response.getBody().asString());
		System.out.println("Status Code: " + response.getStatusCode());
//        AllureLogger.logResponse("Create Resource Response", response);
		logToExtent("Update Attachment (PUT)", newJson, response);
        captureDbSnapshot(AttachmentId, "Update Attachment (PUT)");

	}
    @When("Update Patch Attachment")
	public void Update_Patch_Attachment() {
//		if (MessageId == 0)
...
//        String patchPayload = "{ \"title\": \"" + title + "\" }";
		...
		response = given().header("Content-Type", "application/json").header("X-TENANT-ID", tenantId)
				.header("Author", author).pathParam("id", AttachmentId).body(newJson).when()
				.patch(baseUrl + Attachment_endpoint + "/{id}").then().extract().response();

		System.out.println("PATCH Request Body:\n" + newJson);
		System.out.println("PATCH Response Body:\n" + response.getBody().asString());
		System.out.println("Status Code: " + response.getStatusCode());
//        AllureLogger.logResponse("Create Resource Response", response);
		logToExtent("Update Patch Attachment (PATCH)", newJson, response);
        captureDbSnapshot(AttachmentId, "Update Attachment (PATCH)");

	}
    @When("Delete Attachment with ID")
  	public void delete_Attachment_with_ID() {
  		response = given().header("Content-Type", "application/json").header("X-TENANT-ID", tenantId)
  				.header("Author", author).pathParam("id", AttachmentId).when()
  				.delete(baseUrl + Attachment_endpoint + "/{id}").then().extract().response();

  		System.out.println("Delete Response Body:\n" + response.getBody().asString());
  		System.out.println("Delete Status Code: " + response.getStatusCode());

  		logToExtent("Delete Attachment", null, response);
          captureDbSnapshot(AttachmentId, "Delete Attachment");

  	}
    
    @Then("The Attachment response status code should be {int}")
    public void the_attachment_response_status_code_should_be(Integer statusCode) {
        if (response == null) {
    ...
    @Then("The Attachment response should contain {string}")
    public void the_attachment_response_should_contain(String key) {
        if (response == null) {
            throw new AssertionError("No response available to assert against. Make sure the POST ran.");
        }
